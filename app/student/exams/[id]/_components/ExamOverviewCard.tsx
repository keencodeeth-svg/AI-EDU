  ...
  return (
    <Card title="考试信息" tag="概览">
      <div className="grid grid-2">
        <div className="card feature-card">
          ...
          <div className="pill-list">
            ...
            <span className="pill">{data.class.name} · {SUBJECT_LABELS[data.class.subject] ?? data.class.subject} · {data.class.grade} 年级</span>
          </div>
        </div>
        <div className="card feature-card">
          <EduIcon name="chart" />
          ...
          {submitted ? (
            <div style={{ marginTop: 8, fontSize: 13 }}>成绩：{finalScore}/{finalTotal}</div>
          ) : (
          ...
          {lockReason ? <div style={{ marginTop: 8, fontSize: 12, color: "#b42318" }}>{lockReason}，当前仅可查看作答记录。</div> : null}
        </div>
      </div>
      <div className="cta-row" style={{ marginTop: 12 }}>
        ...
        {!submitted ? (
          <button className="button secondary" type="button" onClick={onSaveDraft} disabled={saving || submitting || lockedByTime || lockedByServer}>
            {saving ? "保存中..." : "保存进度"}
          </button>
        ) : null}
      </div>
