  ...
  return (
    <div className="grid" style={{ gap: 18 }}>
      <div className="section-head">
        <div>
          <h2>课程表与学习日程</h2>
          <div className="section-sub">把固定课程、今日节次和学习任务放到同一视图里，减少来回切页找信息。</div>
        </div>
            ...
            >
              {(schedule?.weekly ?? []).map((day, index) => (
                <div className="card" key={`${day.weekday}-${day.date}`} style={{ minHeight: 220 }}>
                  ...
                  <div className="section-sub">{new Date(`${day.date}T00:00:00`).toLocaleDateString("zh-CN", { month: "numeric", day: "numeric" })}</div>
