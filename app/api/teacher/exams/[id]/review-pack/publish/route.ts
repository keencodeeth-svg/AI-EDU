...
export const POST = withApi(async (request, context) => {
  ...
  const paper = await getExamPaperById(params.id);
  if (!paper) {
    notFound("not found");
  }
  ...
  const questionMap = new Map(
    questions
      .filter((item) => itemQuestionIds.includes(item.id))
      .map((item) => [item.id, item])
  );

  const requestedStudentIds = new Set((body.studentIds ?? []).map((item) => item.trim()).filter(Boolean));
  const candidateStudentIds = Array.from(new Set([...assignmentMap.keys(), ...submissionMap.keys()])).filter((studentId) =>
    requestedStudentIds.size ? requestedStudentIds.has(studentId) : true
  );
